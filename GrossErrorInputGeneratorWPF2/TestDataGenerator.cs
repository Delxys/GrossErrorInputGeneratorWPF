...
{
    internal class TestDataGenerator : IGenerator
    {
        public void Generate(Settings settings)
        {
            ...
            for (int num = 0; num < filesNumber; num++)
            {
                ...
                //int copies = random1.Next(4, 8);
                int size = settings.Multiplier * comonInputFlows + basicInputFlows;
                var errorsIdList = $"errorsIdListTestSetS{size}.txt";

                int numberOfNodes = 3;
                var newVaraibles = new List<Flow>(balanceInput);
                var generatedPath = ($"StringJsonTestSet/generated-outputTestSet{num}.json");
                var generatedDoublesPath = ($"DoublesJsonTestSet/generated-outputDoublesTestSet{num}.json");
                var random = new Random();
                var randomDouble = random.NextDouble();
                var randomMeasuredSum = Math.Round(randomDouble * (550 - 120) + 120, 2);
                ...
                for (int i = 0; i < settings.Multiplier; i++)
                {
                    ...
                    var generatedVariables = randomedBalanceInput.Select(x =>
                    {
                        ...
                        var randomDoubleForTolerance = random.NextDouble();
                        var randomDoubleTolerance = Math.Round(randomDoubleForTolerance * (10.0 - 0.01) + 0.01, 2);
                        ...
                        if (int.Parse(newVariable.Id.Split("-").Last()) % 2 == 1)
                        {
                            ...
                            newVariable.Tolerance = x.Tolerance + randomDoubleTolerance;
                            return newVariable;
                        }
